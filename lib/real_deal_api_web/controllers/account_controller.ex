defmodule RealDealApiWeb.AccountController do
  ...
  defp authorize_account(conn, email, hashed_password) do
    case Guardian.authenticate(email, hashed_password) do
      {:ok, account, token} ->
        ...
        |> put_status(:ok)
        |> json(RealDealApiWeb.AccountJSON.account_token(%{account: account, token: token}))
  ...
  def refresh_session(conn, %{}) do
    ...
    |> Plug.Conn.put_session(:account_id, account.id)
    |> put_status(:ok)
    |> json(RealDealApiWeb.AccountJSON.account_token(%{account: account, token: new_token}))
  end
  ...
  def sign_out(conn, %{}) do
    ...
    |> put_status(:ok)
    |> json(RealDealApiWeb.AccountJSON.account_token(%{account: account, token: nil}))
  end

  def show(conn, %{"id" => id}) do
    account = Accounts.get_full_account(id)
    json(conn, RealDealApiWeb.AccountJSON.full_account(%{account: account}))
  end

  def current_account(conn, %{}) do
    conn
    |> put_status(:ok)
    |> json(RealDealApiWeb.AccountJSON.full_account(%{account: conn.assigns[:account]}))
  end
